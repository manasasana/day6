...
create view salesperson as select count(salesman_id) as persons, city from salesman
group by city;

select salesman_id, name, city, format(commission,'p') as commision from salesman;
...
select m.match_no, c.country_name,m.goal_score from match_details m,  soccer_country c
